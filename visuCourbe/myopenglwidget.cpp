...
void myOpenGLWidget::makeGLObjects()
{
    ...
    if(sizeChanged){
        setSizeChanged();
        points = c.CreateControlPoint(nbCol);
    }
    ...
    QVector<Point> vertices = c.surfBez(points,0.05,nbCol);
...
void myOpenGLWidget::paintGL()
{
    ...
    glPointSize (2.0f);
    glDrawArrays(GL_POINTS, nbCol*nbCol,vertData.size()/6-(nbCol*nbCol));
...
Point myOpenGLWidget::setDeplacementPoint(float u, float v){
    ...
    qDebug() << "test1" << endl;
    Point p = s.bernPoint(points,u,v,nbCol);
    qDebug() << "test2" << endl;
    return p;
}
